...
const DonationCertify = () => {
  ...
  const [_isLoggedIn, _setIsLoggedIn] = useState(
    Boolean(localStorage.getItem("ACCESS_TOKEN"))
  );

  useEffect(() => {
    const onStorage = () =>
      _setIsLoggedIn(Boolean(localStorage.getItem("ACCESS_TOKEN")));
  ...
  const [_guestNickname, _setGuestNickname] = useState("");
  ...
  const handleSubmit = async () => {
    ...
    /*
    if (!_isLoggedIn && !_guestNickname.trim()) {
      alert("닉네임을 입력해 주세요.");
      return;
    }
      */
    ...
    if (!_isLoggedIn) {
      formData.append("nickname", _guestNickname.trim());
    }
      */
    try {
      setLoading(true);
      ...
      <FormSection>
        <div>
          {/*!_isLoggedIn && (
            ...
            value={_guestNickname}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
              _setGuestNickname(e.target.value)
            }
            />
          )*/}
